function varargout = TwoRFT(varargin)
% TWORFT MATLAB code for TwoRFT.fig
%      TWORFT, by itself, creates a new TWORFT or raises the existing
%      singleton*.
%
%      H = TWORFT returns the handle to a new TWORFT or the handle to
%      the existing singleton*.
%
%      TWORFT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TWORFT.M with the given input arguments.
%
%      TWORFT('Property','Value',...) creates a new TWORFT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TwoRFT_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TwoRFT_OpeningFcn via varargin.
%
...
% tishi_edit the above text to modify the response to help TwoRFT

% Last Modified by GUIDE v2.5 29-Jun-2016 18:52:25

% Begin initialization code - DO NOT TISHI_EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TwoRFT_OpeningFcn, ...
                   'gui_OutputFcn',  @TwoRFT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before TwoRFT is made visible.
function TwoRFT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TwoRFT (see VARARGIN)
...
% Choose default command line output for TwoRFT
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TwoRFT wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TwoRFT_OutputFcn(hObject, eventdata, handles) 
...
amax=str2num(get(handles.amax_edit,'String'));
amin=str2num(get(handles.amin_edit,'String'));
a_t=amin:1:amax;
max_a=0;
max_t2=0;
max_sp2=0;
...
for i=1:M
    ...
    tic
    for a_index=1:length(a_t)
        set(handles.Tishi_edit,'String',[num2str(a_index/(L_a)*100),'%']);
        pause(0.0000001);
        BU_a=exp((1j*2*pi*a_t(a_index)*Tr^2.*Bu_m2L.*fc/C));
        pc_result=pc_result.*BU_a;
        pc_result_fft=fft(pc_result,[],2);
        for Nk=1:length(K)
            %%乘以补偿因子补偿多普勒模糊
    ...
    time=toc;
    set(handles.TwoRFTcost_edit,'String',num2str(time));
    set(handles.Tishi_edit,'String','运行结束');
    axes(handles.twoRFT_figure);
    mesh((abs(sp_final2))/max(max(abs(sp_final2))));%
    str_twoRFT=['SNR=',num2str(SNR),'时的二阶RFT结果'];
    title(str_twoRFT,'FontSize',18)
    xlabel('距离单元')
    ylabel('速度单元')
    zlabel('归一化幅度')
