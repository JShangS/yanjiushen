function varargout = Keystone(varargin)
% KEYSTONE MATLAB code for Keystone.fig
%      KEYSTONE, by itself, creates a new KEYSTONE or raises the existing
%      singleton*.
%
%      H = KEYSTONE returns the handle to a new KEYSTONE or the handle to
%      the existing singleton*.
%
%      KEYSTONE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KEYSTONE.M with the given input arguments.
%
%      KEYSTONE('Property','Value',...) creates a new KEYSTONE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Keystone_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Keystone_OpeningFcn via varargin.
%
...
% tishi_edit the above text to modify the response to help Keystone

% Last Modified by GUIDE v2.5 29-Jun-2016 18:27:13

% Begin initialization code - DO NOT TISHI_EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Keystone_OpeningFcn, ...
                   'gui_OutputFcn',  @Keystone_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Keystone is made visible.
function Keystone_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Keystone (see VARARGIN)
...
set(handles.Keystonecost_edit,'String',0);
...
% Choose default command line output for Keystone
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Keystone wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Keystone_OutputFcn(hObject, eventdata, handles) 
...
%%%Chirp_Z%%%%%%%%%%%%%%%%%
max_MTD=0;
MTD_bu_fft=zeros(L,M);
    pc_result_fft=pc_result_fft.';
    tic
    LL=2*M;%LL点Chirp—Z变换
    df=B/L;
    ft=(0:L-1)'*df;%-Fs/2+
   ...
   for i=1:L
    ...
    for k_t=-P:P
        set(handles.Tishi_edit,'String',[num2str((k_t+P)/(2*P+1)*100),'%']);
        pause(0.0000001);
        for i=1:L;
            for j=1:M
                pc_result_bu_t2(i,j)=pc_result_bu_t1(i,:)*exp(1i*2*pi/M*m*(j-M/2-1))*exp(1j*2*pi*k_t*j*(1/(1+eta*i))); %脉冲向ifft
            end
        end
        pc_result_bu_t=ifft(pc_result_bu_t2,[],1);%快时间ifft变换
        %%%keystone后MTD
        MTD_bu_fft_t=(fft(pc_result_bu_t,[],2));%fftshift
        MTD_bu_fft_t=abs(MTD_bu_fft_t);
        if (A1+A2+A3>1)
            bijiao_t=MTD_bu_fft>MTD_bu_fft_t;
            MTD_bu_fft=MTD_bu_fft.*bijiao_t+MTD_bu_fft_t.*(ones(L,M)-bijiao_t);
        else
          max_t=max(max(abs(MTD_bu_fft_t)));
        %比记录的最大峰值大则替换
            if max_t>max_MTD
                max_MTD=max_t;
                pc_result_bu=pc_result_bu_t;
                MTD_bu_fft=MTD_bu_fft_t;
        %     else
        %         break;
            end
        end
   end
           axes(handles.Keystone_figure);
           mesh(abs(MTD_bu_fft(:,1:M))/max(max(abs(MTD_bu_fft(:,1:M)))));
           time=toc;
           str_Keystone=['SNR=',num2str(SNR),'时的Keystone结果'];
           title(str_Keystone,'FontSize',18)
           xlabel('距离单元')
           ylabel('速度单元')
           zlabel('归一化幅度')
           set(handles.Keystonecost_edit,'String',num2str(time));
           set(handles.Tishi_edit,'String','运行结束');
PC_gai=ifft(MTD_bu_fft(:,1:M),[],2);
axes(handles.PC_gai_figure);
imagesc(abs(PC_gai).') 
title('校正后的脉压结果','FontSize',18)
xlabel('距离单元')
ylabel('脉冲单元')
